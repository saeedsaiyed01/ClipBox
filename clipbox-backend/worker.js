...
function processVideo(job) {
  const { videoPath, background } = job.data;
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.stderr.on('data', (data) => {
      // Don't log *every* single progress line, just for key info
      if (data.toString().includes('frame=')) {
        // You could parse progress here if you wanted
      } else {
    ...
    ffmpeg.on('close', (code) => {
      if (code === 0) {
        ...
        // This URL works because the 'public' folder is served by the API
...
const worker = new Worker('video-processing', async (job) => {
  ...
  return { finalUrl }; // This result is saved to job.returnvalue
}, {
  connection: { host: 'localhost', port: 6379 } // 'redis' is the service name
});
